...
const Product = () => {
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get("/port/api/v1/product/read");
    ...
    const handleDelete = async (productId) => {
        try {
            const response = await axios.delete(
                `/port/api/v1/product/deleteById/${productId}`
            );
            if (response.status === 200) {
            ...
            >
                <form onSubmit={handleSubmit} className="w-[40vw] xs:w-full">
                    ...
                    <div>
                        ...
                        <button onClick={handleAddModalClose} className="px-3 py-2 rounded-md bg-red-500 text-white mx-2">Close</button>
                    </div>
                </form>
            </Modal>
...
export function EditProducts({
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await fetch(`/port/api/v1/product/${productId}/image`, {
                method: "POST",
                body: formData,
            });
    ...
    const handleRemoveImage = async (imageIndex) => {
        try {
            const response = await axios.delete(
                `/port/api/v1/product/${productId}/images/${imageIndex}`
            );
            if (response.status === 200) {
        ...
        const fetchProductById = async () => {
            try {
                const response = await axios.get(
                    `/port/api/v1/product/readById/${productId}`
                );
                if (response.status === 200) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(
                `/port/api/v1/product/editAndUpdate/${productId}`,
                {
                    method: "PUT",
                    headers: {
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="px-5 py-2 text-white bg-blue-500 rounded-lg">Update</button>
                <button onClick={handleEditModalClose} className="px-3 py-2 rounded-lg bg-red-500 text-white mx-2">Cancel</button>
            </form>
        </>
    );
}
