...
function Profile() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch("/port/api/v1/user/avatar", {
                method: "POST",
                headers: { Authorization: `Bearer ${accessToken}` },
                body: formData,
            });
    ...
    const handleUpdateProfile = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch("/port/api/v1/user/update", {
                method: "PUT",
                headers: {
            ...
            <div className="overflow-scroll py-[10vh] px-[5vw] ">
                ...
                {profileData && (
                    <main className=" xs:p-1  ">
                        <form onSubmit={handleUpdateProfile} className="space-y-5 max-w-4xl mx-auto">
                            {isLoading && <Loader />}
                            ...
                            <section className="grid grid-cols-1 sm:grid-cols-3 gap-8 place-content-center">
                                <div className="sm:col-span-1 flex-col gap-10 bg-slate-200 p-4 rounded-xl flex justify-center items-center">
                                    ...
                                    >
                                        ...
                                        <img
                                            src={editedProfileData.profilePicture || "default-image-url"}
                                            alt="Profile"
                                ...
                                <div className="sm:col-span-2 bg-slate-200 p-4 rounded-xl">
                                    <div className="space-y-5">
                                        {["name", "email", "username", "phoneNumber", "address"].map((field) => (
                                            <div key={field}>
                                                    ...
                                                    <div className="order-details">
                                                        <ul className="list-disc ml-4 mb-2">
                                                            {order.products.map((product, productIndex) => (
                                                                <li key={`${product.product}-${productIndex}`} className="border rounded-lg p-4 mb-4">
                                                                    ...
                                                                    <div className="text-gray-600 mt-2">Total Price: ${product.totalPrice}</div>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                        <div className="mb-2">
                                                        ...
                                                        <div className="mb-2">
                                                            {order.payment.transactionType === 'Online' ? (
                                                                <>
                                                                    <p><span className="font-bold">Transaction Type:</span> {order.payment.transactionType}</p>
                                                                    <p><span className="font-bold">Transaction ID:</span> {order.payment.transactionId}</p>
                                                                    <p><span className="font-bold">Bank Account Name:</span> {order.payment.bankAccountName}</p>
                                                                </>
                                                            ) : (
                                                                <p><span className="font-bold">Transaction Type:</span> {order.payment}</p>
                                                            )}
                                                        </div>
                                                        <div>
