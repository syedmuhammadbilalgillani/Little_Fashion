...
const Forget = () => {
    ...
    const handleSubmit = async (action) => {
        ...
        try {
            ...
            switch (action) {
                case 'generateOTP':
                    endpoint = '/port/api/v1/user/generateAndSendOTP';
                    message = 'OTP generated successfully';
                    break;
                case 'verifyOTP':
                    endpoint = '/port/api/v1/user/verifyOTP';
                    message = 'OTP verified successfully';
                    break;
                case 'updatePassword':
                    endpoint = '/port/api/v1/user/updatePassword';
            ...
            console.error(error);
            toast.error(`Failed to ${action === 'generateOTP' ? 'generate' : action === 'verifyOTP' ? 'verify' : 'update'} OTP`);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className='p-5'>
            ...
            <div className='space-y-5'>
                <input
                    ...
                    placeholder={step === 0 ? 'Email' : step === 1 ? 'Enter OTP' : 'New Password'}
                    value={step === 0 ? email : step === 1 ? otp : newPassword}
                    onChange={handleInputChange}
                />
                <button
                    onClick={() => handleSubmit(step === 0 ? 'generateOTP' : step === 1 ? 'verifyOTP' : 'updatePassword')}
                    type="submit"
                    disabled={isLoading}
                    className="uppercase bg-black hover:bg-[--red] py-[max(1vw,1rem)] text-white text-[max(1vw,1rem)] rounded-[max(2vw,10rem)] w-full"
                >
                    {isLoading ? <Loader /> : step === 0 ? 'Generate OTP' : step === 1 ? 'Verify OTP' : 'Update Password'}
                </button>
            </div>
        </div>
    );
