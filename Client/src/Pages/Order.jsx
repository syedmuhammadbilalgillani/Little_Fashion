...
const Order = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch("/port/api/v1/payment/readBankAccountDetails", {
                    method: "GET",
                    headers: {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const response = await fetch("/port/api/v1/order/checkout", {
                method: "POST",
                headers: {
                    ...
                    <div className="col-span-1 xs:col-span-2 sm:col-span-2">
                        <div className="flex flex-wrap gap-[3vw]">
                            {cartItems.map((item, index) => (
                                <div className="p-5 rounded-lg shadow-md hover:bg-gradient-to-r hover:from-gray-100 hover:to-orange-200 w-full max-w-[300px]" key={index}>
                                    <h2 className="text-lg font-semibold mb-2">Product Name: {item.product.name}</h2>
                                    <p className="text-gray-700 mb-2">Quantity: {item.quantity}</p>
                                    <p className="text-gray-700">Price: ${item.product.price * item.quantity}</p>
                                </div>
                            ))}
                        </div>

                        <div className="my-10">
                            <h2 className="text-xl font-bold mb-2">Total Price: ${totalPrice}</h2>
                        </div>
                        <div className="mb-4">
                            <h1 className="text-2xl font-bold text-center mb-8">Enter Detail</h1>
                            <input
                                type="text"
                                name="name"
                                placeholder="Name"
